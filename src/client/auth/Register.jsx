...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus('Loading...');
    try {
      const res = await api.apiClient.post('/client/auth/register', { username, email, password });
      setStatus('Success: ' + (res.data?.message || 'Registered!'));
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F19] transition-colors duration-500 font-sans text-slate-800 dark:text-slate-200 flex items-center justify-center relative py-12 px-4 sm:px-6 lg:px-8 overflow-hidden selection:bg-indigo-500 selection:text-white">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
         <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-500/20 dark:bg-indigo-600/10 blur-[120px]"></div>
