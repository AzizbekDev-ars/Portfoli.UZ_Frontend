...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F19] transition-colors duration-500 font-sans text-slate-800 dark:text-slate-200 flex items-center justify-center relative py-12 px-4 sm:px-6 lg:px-8 overflow-hidden selection:bg-indigo-500 selection:text-white">
      
      {/* Top Navbar items floating */}
      <div className="absolute top-6 left-6 flex items-center gap-3">
        <Link to="/" className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-indigo-500/20 hover:scale-105 transition-transform">
          P
        </Link>
      </div>
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
         <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-500/20 dark:bg-indigo-600/10 blur-[120px]"></div>
         <div className="absolute bottom-[0%] left-[-10%] w-[40%] h-[40%] rounded-full bg-purple-500/20 dark:bg-purple-600/10 blur-[120px]"></div>
      </div>
      ...
      >
        ...
        <div className="text-center mt-6">
          <span className="text-slate-500 dark:text-slate-400 text-sm">{t.auth.noAccount} </span>
          <Link to="/register" className="font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 transition-colors text-sm">
            {t.auth.toRegister}
          </Link>
        </div>
      </motion.div>
