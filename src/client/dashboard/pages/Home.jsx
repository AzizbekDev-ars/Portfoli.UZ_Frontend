...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [msgsRes, visitorsRes] = await Promise.all([
          api.get('/message'),
          api.get('/visitor')
        ]);
        ...
        setStats({
          ...
          unique: visitorData.uniqueVisitors || 0, // Fix: Changed to total unique visitors
          messages: msgs.length, // Fix: Changed to total messages
          cvDownloads: 0 
        });
      } catch (err) {
