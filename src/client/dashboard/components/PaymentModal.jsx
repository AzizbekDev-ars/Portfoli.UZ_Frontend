...
const PaymentModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handlePay = async (e) => {
    ...
    setLoading(true);
    try {
      // Haqiqiy to'lov API so'rovi
      const res = await api.post('/transaction/pay-pro', {
        cardNumber,
        expiry,
        cvv
      });
      ...
      if (res.data.success) {
        ...
        alert(res.data.message || "To'lovda xatolik");
      }
    } catch (err) {
      alert(err.response?.data?.message || "Server bilan bog'lanishda xatolik");
    } finally {
      setLoading(false);
    }
  };
