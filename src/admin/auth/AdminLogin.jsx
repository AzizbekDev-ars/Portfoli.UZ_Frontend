...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.apiClient.post('/auth/login', { email, password });
      
      if (res.data?.user?.role !== 'admin') {
        setStatus('Error: Siz admin emassiz!');
        return;
      }
      ...
      >
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Login (Email yoki Username)</label>
              <input
                type="text" required
