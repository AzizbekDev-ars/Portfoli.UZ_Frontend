...
const AdminUsers = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.apiAdmin.get(`/admin/users?search=${searchTerm}&filter=${filterStatus}&page=${currentPage}&limit=${itemsPerPage}`);
  ...
  useEffect(() => {
    fetchData();
  }, [searchTerm, filterStatus, currentPage, sortOrder]);

  // --- HANDLERS ---
  const togglePro = async (userId, currentProStatus, e) => {
    e.stopPropagation();
    try {
      await api.apiAdmin.put(`/admin/users/${userId}`, { isPro: !currentProStatus });
      fetchData();
    } catch (err) {
  ...
  const deleteUser = async (userId, e) => {
    ...
    if(window.confirm('Haqiqatdan ham ushbu foydalanuvchini o\'chirmoqchimisiz?')) {
      try {
        await api.apiAdmin.delete(`/admin/users/${userId}`);
        fetchData();
      } catch (err) {
        alert("Xatolik yuz berdi");
      }
                ...
                {users.map((user) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-11 h-11 rounded-xl overflow-hidden border border-white/10 group-hover:border-rose-500/50 transition-all duration-300">
                           <img src={user.profileImage?.url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.username}`} alt={user.username} className="w-full h-full object-cover" />
                        </div>
                        <div>
                          <p className="font-bold text-slate-100 group-hover:text-rose-400 transition-colors text-sm">{user.username}</p>
                    ...
                    <td className="px-6 py-4 text-xs text-slate-400 font-medium">
                       <span className="bg-white/5 px-2.5 py-1 rounded-lg border border-white/5">{new Date(user.createdAt).toLocaleDateString()}</span>
                    </td>
                    <td className="px-6 py-4">
                      {user.isPro ? (
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2.5 opacity-60 group-hover:opacity-100 transition-all">
                        ...
                        <button 
                          onClick={(e) => togglePro(user._id, user.isPro, e)}
              ...
              <div className="px-8 pb-10 -mt-12 flex-1 overflow-y-auto custom-scrollbar">
                <div className="flex flex-col md:flex-row gap-6 items-center md:items-end mb-8">
                  <div className="w-32 h-32 rounded-2xl overflow-hidden border-4 border-[#1a1f2e] shadow-xl bg-[#1a1f2e]">
                     <img 
                      src={selectedUser.profileImage?.url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedUser.username}`} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
                  <div className="md:col-span-3 space-y-6">
                    <section>
                      ...
                      <p className="text-slate-300 text-sm leading-relaxed bg-white/5 p-4 rounded-xl border border-white/5 italic">
                        {selectedUser.profileData?.bio || 'Ma\'lumot kiritilmagan.'}
                      </p>
                    </section>
                    ...
                    <section>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedUser.socialLinks && Object.entries(selectedUser.socialLinks).map(([platform, link], i) => (
                          link && (
                            <div key={i} className="px-3 py-1.5 bg-white/5 rounded-lg border border-white/5 text-[11px] font-bold text-slate-300">
                              <span className="text-rose-500 mr-1">{platform}:</span>
                              {link}
                            </div>
                          )
                        ))}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-4 bg-indigo-500/5 rounded-xl border border-indigo-500/10">
                        ...
                        <p className="text-white text-xs font-bold">{selectedUser.selectedTemplate || 'Default'}</p>
                      </div>
                      <div className="p-4 bg-purple-500/5 rounded-xl border border-purple-500/10">
                        ...
                        <p className="text-white text-xs font-bold">Formal</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { label: 'Tashriflar', value: selectedUser.stats?.visitors || 0, color: 'blue' },
                        { label: 'Xabarlar', value: selectedUser.stats?.messages || 0, color: 'emerald' },
                        { label: 'Certifikatlar', value: selectedUser.stats?.certificates || 0, color: 'amber' },
                        { label: 'Loyihalar', value: selectedUser.stats?.projects || 0, color: 'indigo' },
                        { label: 'Tajriba', value: selectedUser.stats?.experience || 0, color: 'purple' },
                      ].map((stat, idx) => (
