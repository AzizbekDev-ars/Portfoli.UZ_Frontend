...
const AdminHome = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [statsRes, usersRes, newsRes] = await Promise.all([
        ...
        api.apiAdmin.get(`/admin/users?search=${searchTerm}&filter=${userFilter}&page=${currentPage}&limit=${itemsPerPage}`),
        api.apiAdmin.get('/admin/announcements')
      ]);
  ...
  useEffect(() => {
    fetchData();
  }, [searchTerm, userFilter, currentPage]);

  // --- HANDLERS ---
  const togglePro = async (userId, currentProStatus) => {
    try {
  ...
  const handleAddNews = async (e) => {
    ...
    try {
      await api.apiAdmin.post('/admin/announcements', newsForm);
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {[
          ...
          { label: 'Umumiy Loyihalar', value: stats.totalProjects || 0, icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', color: 'cyan' },
          { label: 'Xabarlar', value: stats.totalMessages || 0, icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z', color: 'purple' },
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <img src={user.profileImage?.url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.username}`} alt={user.username} className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-white/10 bg-white/5" />
                        ...
                        <td className="px-6 py-4 text-slate-400 text-xs md:text-sm">{new Date(user.createdAt).toLocaleDateString()}</td>
                        <td className="px-6 py-4">
                          {user.isPro ? (
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-1 md:gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                            <button 
                              onClick={() => togglePro(user._id, user.isPro)}
