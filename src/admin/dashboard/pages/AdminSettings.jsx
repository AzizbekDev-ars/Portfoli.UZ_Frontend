...
const AdminSettings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const [statsRes, settingsRes, meRes] = await Promise.all([
                api.apiAdmin.get('/admin/stats'),
                api.apiAdmin.get('/admin/settings'),
                api.apiClient.get('/auth/me')
            ]);
            setBalance(statsRes.data.totalRevenue - statsRes.data.totalExpenses);
            
            const s = settingsRes.data;
            setSubData({ proPrice: s.proPrice, freeProjectsLimit: s.freeProjectsLimit, freeCertLimit: s.freeCertLimit });
            setSeoData({ siteName: s.siteName, metaDesc: s.metaDesc, bannerActive: s.bannerActive, bannerText: s.bannerText });
  ...
  const [subData, setSubData] = useState({ proPrice: 0, freeProjectsLimit: 0, freeCertLimit: 0 });
  const [apiData, setApiData] = useState({ cloudinaryUrl: '', ipapiKey: '', smtpHost: '', smtpPort: 0, smtpEmail: '', smtpPassword: '' });
  const [seoData, setSeoData] = useState({ siteName: '', metaDesc: '', bannerActive: false, bannerText: '' });
  ...
  const handleSave = async (e) => {
    ...
    try {
        if (activeTab === 'security') {
            ...
            // Admin profilini yangilash (email va parol)
            await api.apiClient.put('/auth/user', updatePayload);
        } else {
            let payload = {};
            ...
            await api.apiAdmin.put('/admin/settings', payload);
        }
                    ...
                    <motion.div key="security" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                      <h2 className="text-lg font-bold text-white flex items-center gap-2">Xavfsizlik Sozlamalari</h2>
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        <div>
                          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Login (Username)</label>
                          <input type="text" value={securityData.username} onChange={e => setSecurityData({...securityData, username: e.target.value})} className="w-full bg-black/30 border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-rose-500 outline-none transition-all" required />
                        </div>
                        <div>
                          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Email</label>
                          <input type="email" value={securityData.email} onChange={e => setSecurityData({...securityData, email: e.target.value})} className="w-full bg-black/30 border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-rose-500 outline-none transition-all" required />
                        </div>
                      </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
